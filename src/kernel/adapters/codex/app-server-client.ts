 ...
 * for thread management, turn execution, and thread resume.
 *
...
export interface AppServerClientOptions {
  ...
  /** Called for each line written to codex stderr (useful for debug logging) */
...
export interface ThreadStartParams {
  ...
  personality?: string;
  /** Session-level developer instructions bound at thread creation */
...
export interface ThreadResumeParams {
  threadId: string;
  /** Session-level developer instructions rebound at thread resume */
  developerInstructions?: string;
}
...
export class CodexAppServerClient extends EventEmitter {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      // Forward stderr line-by-line to the caller, or drain it so the
      // child process never blocks on a full pipe buffer.
      if (this.onStderr && this.process.stderr) {
        const stderrRl = createInterface({ input: this.process.stderr });
        stderrRl.on("line", (line) => this.onStderr!(line));
      } else {
  ...
  async threadResume(params: ThreadResumeParams): Promise<Record<string, unknown>> {
