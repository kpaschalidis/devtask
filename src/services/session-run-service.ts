...
export function listWorkPhaseSessions(paths: DevtaskPaths, workId: string): PhaseSessionSummary[] {
  ...
  const managedPhases: SessionPhase[] = ["orchestrate", "repo-plan", "review", "execute", "ci-fix"];
...
function toPhaseSessionSummary(run: SessionRun): PhaseSessionSummary {
  return {
    phase: run.phase,
    workId: run.workId,
    repoId: run.repoId,
...
const PHASES: SessionPhase[] = ["orchestrate", "repo-plan", "execute", "review", "ci-fix", "compound"];
