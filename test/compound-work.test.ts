...
const kernelMocks = vi.hoisted(() => ({
  runKernelOneShot: vi.fn(async (_config, _options, prompt: string, output: { outputPath: string }) => {
...
describe("compound work", () => {
  it("exposes only the human-readable learning report as its output artifact", async () => {
    ...
    expect(kernelMocks.runKernelOneShot.mock.calls[0]?.[1].addDirs).toEqual([workItemDir(paths, item.id)]);
  });
});
